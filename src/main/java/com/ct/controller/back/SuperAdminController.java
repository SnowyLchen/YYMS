...
public class SuperAdminController {
    ...
    public String addSuperAdmin(SuperAdmin superAdmin, HttpServletRequest request){
        //获取superadmin表中的总数据记录数，大于1条时拒绝插入
        int row=superAdminService.querySuperAdminCount();
        if(row>0){
            request.setAttribute("REGISTER_ERROR","无法初始化");
    ...
    public String login(SuperAdmin superAdmin,HttpServletRequest request, HttpSession session){
            ...
            if (superAdmin != null) {
                 ...
                 if (superAdmin==null){
                     ...
                     //存储管理员开发者信息
                     session.setAttribute("USER_INFO", "超级管理员");
                     return "redirect:/SuperAdmin/ToLogin";
                 }
            } else {
                //存储错误信息
    ...
    public String llo(HttpSession session, HttpServletRequest request){
        //先判断是否有管理员登陆
